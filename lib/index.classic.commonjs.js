...
function parseStyles(result, styles, options, state) {
  ...
  return Promise.resolve(statements).then(promiseEach(function (stmt) {
    ...
    if ("import" !== stmt.type || !options.resolveUrls && URL_RE.test(stmt.uri)) return null;else return resolveImportId(result, stmt, options, state);
  })).then(function () {
    var imports = [],
        bundle = [];
...
function resolveImportId(result, stmt, options, state) {
  var atRule = stmt.node,
      ...
      sourcePath = options.resolveUrls && URL_RE.test(sourceFile) ? sourceFile : path.dirname(sourceFile),
      base = sourceFile ? sourcePath : options.root;
